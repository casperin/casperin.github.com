...
function isType(type, x){
  ...
  switch (arguments.length){
    case 2: return main(x);
    case 1: return main;
    case 0: throw new Error('missing arguments');
  }
}
...
// TODO: AutoCurry these properly + foldl1 and foldr1
var foldl = autoCurry(function(fn, memo, arr){
...
var foldr = autoCurry(function(fn, memo, arr){
  var i = arr.length;
  while (i--)
    memo = fn(memo, arr[i]);
  return memo;
...
var zipWith = autoCurry(function(fn, arr, arr2){
  var result = [], i = -1, len = Math.min(arr.length, arr2.length);
  while (++i < len)
    result.push( fn(arr[i], arr2[i]) );
  return result;
});
